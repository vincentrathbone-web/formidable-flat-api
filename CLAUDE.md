...
`Draft Status`/`Entry Status` (the latter is the source-qualified `Form N: Entry Status` in `item_system_field_definitions()`) come from `frm_items.is_draft` via the shared `draft_status_label()` helper — `0` → `Submitted`, `1` → `Draft`, `2` → `Abandoned`, anything else → `Status N` (never blank). `0`/`1` are Formidable core's own `FrmEntriesHelper::SUBMITTED_ENTRY_STATUS`/`DRAFT_ENTRY_STATUS`; `2` is a Pro-only status (partial entries) the free core never defines. Before this helper existed, three near-identical inline `match()` blocks (in `fetch_form_rows()`, `fetch_view_rows()`, `fetch_merged_rows()`) each mapped `0` to `''` — meaning every normal, fully-submitted entry (the overwhelming majority of rows in any real form) showed a *blank* status, not "Submitted". Worse, `fetch_merged_rows()`/`fetch_form_rows()`/`fetch_view_rows()` all skip non-zero-status rows by default (`if (!$include_drafts && $status != 0) continue;`), so with drafts excluded — the default — *every row that ever reached the status column* had status `0`, making the column look 100% broken rather than just wrong for drafts. If you touch `draft_status_label()`, keep the `0 => 'Submitted'` mapping — reintroducing a blank default there silently recreates this exact bug.
...
[ 'slug', 'label', 'tables' => [ ['form_id', 'key_field_id'] ],
  ...
  'joins' => [ ['query_slug','left_key','right_key','match',      // v2.27.0+
                'join_type',                                      // v3.2.1 — left|inner|right|full, match:first/all only
